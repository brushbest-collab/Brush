...
/* 解析可寫入的 python 資料夾（打包後在 app.asar.unpacked/python） */
function getPythonDir() {
  if (isDev) return path.join(__dirname, 'python');
  const base = process.resourcesPath; // ...\resources
  const unpacked = path.join(base, 'app.asar.unpacked');
  const devFallback = path.join(base, 'app'); // 某些打包方式會是 resources/app
  const p1 = path.join(unpacked, 'python');
  const p2 = path.join(devFallback, 'python');
...
function hasPbsOk() {
  try {
    const p = path.join(getPythonDir(), 'pbs', 'ok');
...
// 跟隨 301/302/307/308；支援 Range 續傳
...
// 取某個 Tag 的 assets，過濾 prefix，依檔名排序（.001 -> .nnn）
async function getReleaseParts(owner, repo, tag, prefix) {
  ...
  if (!list.length) throw new Error(`No assets matched prefix "${prefix}" on ${owner}/${repo}@${tag}`);
...
ipcMain.handle('get-status', async () => {
  ...
  return {
    pythonDir, modelDir,
    hasPbs: hasPbsOk(),
    hasModel: hasModelDir(),
    isDev
  };
});

// 下載整包分卷 +（可選）自動解壓
ipcMain.handle('download-model', async (e) => {
  try {
    const pythonDir = getPythonDir();
